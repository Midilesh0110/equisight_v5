# src/alpha_engine.py — Multi‑Pair Statistical Arbitrage Engine
import numpy as np
import pandas as pd
...
class PairConfig:
    def __init__(self, stock_a: str, stock_b: str, hedge_window: int = 60,
                 z_score_window: int = 20, entry_z: float = 1.5, exit_z: float = 0.0):
...
class PairsAlphaEngine:
    ...
    def compute_signal(self, pair_cfg: PairConfig, raw_data: Dict[str, pd.DataFrame],
        ...
        if z < -pair_cfg.entry_z:
            return {'action': 'BUY_SPREAD', 'z_score': z, 'beta': beta}
        elif z > pair_cfg.entry_z:
            return {'action': 'SELL_SPREAD', 'z_score': z, 'beta': beta}
        return None
